...
export default function ServicePage({ params }: PageProps) {
  ...
  const loadData = async () => {
       ...
       try{
        ...
        const filteredProviders = data.providers.filter((eachItem: any)=>eachItem.services.some((service: string)=>service.toLowerCase().includes(slug.replace(/-/g, ' ').toLowerCase())));
           ...
           {"Click here to post project"} →
...

            {
                !loading && !failed && matchedServiceProviders.length===0 &&(
                    ...
                    <h1 className="text-center font-semibold text-3xl mt-20 mb-20">No service providers found for {slug.replace(/-/g, ' ')}</h1>
                        </div>
                )
            }
           </main>
