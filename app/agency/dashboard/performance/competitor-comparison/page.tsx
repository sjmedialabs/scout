...
const CompetitorComparisonPage = () => {
  ...
  const loadData=async()=>{
    ...
    setFailed(false);
    try{
      const[providerRes, searchesRes,proposalRes,requirementRes,reviewsRes]=await Promise.all([
      ...
      if(!providerRes.ok || !searchesRes.ok || !proposalRes.ok || !requirementRes.ok || !reviewsRes.ok){
        throw new Error("Failed to fetch data");
      }
      const[providerData, searchesData, proposalsData,requirementsData,reviewData]=await Promise.all([
        providerRes.json(),
        searchesRes.json(),
        proposalRes.json(),
        requirementRes.json(),
      ...
      let viewRate=(providerData.provider?.lastMonthProfileViews !==0)
      ?
      Math.round(
        ((providerData.provider.currentMonthProfileViews - providerData.provider?.lastMonthProfileViews)/providerData.provider?.lastMonthProfileViews)*100
      )
      :
      0;
      ...
      setProfilePerformanceStats({
        ...
        currentMonthProfileViews:providerData.provider.currentMonthProfileViews,
      ...
      setLeadsGeneratedStats({
        ...
        leads:proposalsData.proposals.filter((proposal:any)=>proposal.status.toLowerCase()==="accepted" || proposal.status.toLowerCase()==="completed").length
      })

      // conversion funnel stats

      let totalProposals=proposalsData.proposals.length;
      let views=proposalsData.proposals.filter((proposal:any)=>proposal.clientViewed).length;
      let contacts=proposalsData.proposals.filter((proposal:any)=>proposal.status.toLowerCase()!=="pending").length;
      let won=proposalsData.proposals.filter((proposal:any)=>proposal.status.toLowerCase()==="accepted" || proposal.status.toLowerCase()==="completed").length;
      ...
      console.log("Won:",won);
       setConversionFunnelStats({
        views:views,
        viewsPercenntage:totalProposals!==0?(views/totalProposals*100).toFixed(1) as unknown as number:0,
        contacts:contacts,
        contactsPercentage:totalProposals!==0?(contacts/totalProposals*100).toFixed(1) as unknown as number:0,
        proposals:totalProposals,
        proposalsPercentage:totalProposals!==0?(totalProposals/totalProposals*100).toFixed(1) as unknown as number:0,
        won:won,
        wonPercentage:totalProposals!==0?(won/totalProposals*100).toFixed(1) as unknown as number:0
       })

       // review stats
        let totalReviews=reviewData.reviews.length;
        let fiveStarCount=reviewData.reviews.filter((review:any)=>review.rating>=4.1).length;
        let fourStarCount=reviewData.reviews.filter((review:any)=>review.rating>=3.1 && review.rating<=4.0).length;
        let threeStarCount=reviewData.reviews.filter((review:any)=>review.rating>=2.1 && review.rating<=3.0).length;
        let twoStarCount=reviewData.reviews.filter((review:any)=>review.rating>=1.1 && review.rating<=2.0).length;
        let oneStarCount=reviewData.reviews.filter((review:any)=>review.rating<=1.0).length;
      ...
      const monthlyReviewsCount = monthlyReviews.length;
        ...
        setReviewStats({
          ...
          fiveStarPercentage:totalReviews!==0?(fiveStarCount/totalReviews*100).toFixed(1) as unknown as number:0,
          fourStarCount:fourStarCount,
          fourStarPercentage:totalReviews!==0?(fourStarCount/totalReviews*100).toFixed(1) as unknown as number:0,
          threeStarCount:threeStarCount,
          threeStarPercentage:totalReviews!==0?(threeStarCount/totalReviews*100).toFixed(1) as unknown as number:0,
          twoStarCount:twoStarCount,
          twoStarPercentage:totalReviews!==0?(twoStarCount/totalReviews*100).toFixed(1) as unknown as number:0,
          oneStarCount:oneStarCount,
          oneStarPercentage:totalReviews!==0?(oneStarCount/totalReviews*100).toFixed(1) as unknown as number:0,
          ...
          (
            ...
            <CardContent className="space-y-4">
              <div className="text-center mt-10">
                <p className="text-gray-500  text-xl"> No Services matched with the client searches </p>
              </div>
            </CardContent>
            </div>
          )
