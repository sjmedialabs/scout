    ...
    <div className="w-full min-w-0">
      <Select open={open} onOpenChange={setOpen} value={value}>
        ...
        <SelectContent
          ...
          align="start"
          sideOffset={8}
          className={`p-0 rounded-2xl ${contentClassName}`}
        >
          <div className="flex">
            {/* LEFT: Subcategories */}
            <div
              className={`min-w-[220px] border-r ${leftPanelClassName}`}
            >
            ...
            {activeSubCategory && (
              <div
                className={`min-w-[240px] ${rightPanelClassName}`}
              >
                {activeSubCategory.items.map((item) => (
                  <div
                    key={item.slug}
                    onClick={() => {
                      onChange(item.title); // 🔥 send to parent
                      setOpen(false);
                    }}
                    className="px-4 py-2 cursor-pointer text-sm hover:bg-gray-100"
                  >
